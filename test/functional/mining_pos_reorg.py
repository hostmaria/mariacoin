...
class ReorgStakeTest(MariaTestFramework):
    ...
    def check_money_supply(self, expected_mari):
        # verify that nodes have the expected MARI supply
        mari_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(mari_supply, [DecimalAmt(expected_mari)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that MARI supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check MARI supply...")
