...
`test/functional/test_framework/test_framework.py:MariaTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/maria_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_mariad = config["components"].getboolean("ENABLE_BITCOIND")
    ...
    if not (enable_wallet and enable_utils and enable_mariad):
        print("No functional tests to run. Wallet, utils, and mariad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, keep_cache="rewrite"):
    # Warn if mariad is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "mariad"]) is not None:
            print("%sWARNING!%s There is already a mariad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/mariad' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/maria-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie mariads, we can apply a
...
class RPCCoverage():
    ...
    commands per `maria-cli help` (`rpc_interface.txt`).
