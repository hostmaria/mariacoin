...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/MARIAParams"
    else
        PARAMS_DIR="$HOME/.maria-params"
    fi
fi
...
Failed to fetch the MARIA zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
MARIA - fetch-params.sh

This script will fetch the MARIA zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MARIA zkSNARK parameters. Note that it is
