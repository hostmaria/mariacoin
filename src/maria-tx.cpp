...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " maria-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  maria-tx [options] <hex-tx> [commands]  Update hex-encoded maria transaction\n" +
                               "or:     maria-tx [options] -create [commands]   Create hex-encoded maria transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded maria transaction
