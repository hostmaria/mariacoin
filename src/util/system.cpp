...
const char * const MARIA_CONF_FILENAME = "maria.conf";
const char * const MARIA_PID_FILENAME = "maria.pid";
const char * const MARIA_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mariad -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MARIA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MARIA
// Mac: ~/Library/Application Support/MARIA
// Unix: ~/.maria
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MARIA";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MARIAParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MARIAParams
    // Mac: ~/Library/Application Support/MARIAParams
    // Unix: ~/.maria-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MARIAParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "MARIAParams";
#else
    // Unix
    return pathRet / ".maria-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/maria/sapling-spend.params";
        sapling_output = "/usr/share/maria/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/maria/sapling-spend.params";
            sapling_output = "/usr/local/share/maria/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", MARIA_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MARIA_PID_FILENAME));
