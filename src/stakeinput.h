...
public:
    ...
    virtual bool IsZMARI() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
};


class CMariStake : public CStakeInput
{
private:
...
public:
    CMariStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CMariStake* NewMariStake(const CTxIn& txin, int nHeight, uint32_t nTime);
    ...
    bool IsZMARI() const override { return false; }
};
