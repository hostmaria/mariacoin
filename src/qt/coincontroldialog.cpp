...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select MARI Outputs to Spend" :
            "Select Shielded MARI to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::MARI;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show MARIA address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
