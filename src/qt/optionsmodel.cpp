...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MARI);
    ...
    if (settings.contains("nAnonymizeMariaAmount") || reset)
        gArgs.SoftSetArg("-anonymizemariaamount", settings.value("nAnonymizeMariaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in maria.cpp during shotdown to also remove the window positions
