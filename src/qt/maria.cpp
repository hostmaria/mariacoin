...
#include "qt/maria/splash.h"
#include "qt/maria/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/maria/governancemodel.h"
#include "qt/maria/mnmodel.h"
...
{
    return QCoreApplication::translate("maria-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in maria.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in maria.qrc)
...
/** Class encapsulating MARIA Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (MARIAGUI)
...
{
    window = new MARIAGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MARIAGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MARIAGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(MARIAGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MARIAGUI::DEFAULT_WALLET);
        ...
        // MARIA: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MARIAGUI::handlePaymentRequest);
        connect(window, &MARIAGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. MARIA can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(maria_locale);
    Q_INIT_RESOURCE(maria);
    ...
    /// 6. Determine availability of data and blocks directory and parse maria.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", MARIA_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // maria: links repeatedly have their payment requests routed to this process:
