...
#include "qt/maria/send.h"
#include "qt/maria/forms/ui_send.h"
#include "qt/maria/addnewcontactdialog.h"
#include "qt/maria/qtutils.h"
#include "qt/maria/sendchangeaddressdialog.h"
#include "qt/maria/optionbutton.h"
#include "qt/maria/sendconfirmdialog.h"
#include "qt/maria/guitransactionsutils.h"
#include "qt/maria/loadingdialog.h"
...
SendWidget::SendWidget(MARIAGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a MARIA URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-mari");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onMARISelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onMARISelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MARIs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield MARIs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent MARIs to shield."));
    }
}
...
void SendWidget::onMARISelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto mariAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(mariAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(mariAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(mariAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
