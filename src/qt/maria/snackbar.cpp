...
#include "qt/maria/forms/ui_snackbar.h"
#include "qt/maria/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MARIAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MARIAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
