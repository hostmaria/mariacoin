...
#include "qt/maria/dashboardwidget.h"
#include "qt/maria/forms/ui_dashboardwidget.h"
#include "qt/maria/sendconfirmdialog.h"
#include "qt/maria/txrow.h"
...
DashboardWidget::DashboardWidget(MARIAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMari, "square-chart-mari");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelMari, "text-chart-mari");
    ...
    setCssProperty(ui->labelAmountMari, "text-stake-mari-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MARIAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MARI")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMari = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MARI, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mari = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMari += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMari.append(mari);
        chartData->valuesMN.append(mn);

        int max = std::max(mari, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesMari);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMari > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountMari, "text-stake-mari");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountMari, "text-stake-mari-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountMari, ui->labelAmountMN});
    ui->labelAmountMari->setText(GUIUtil::formatBalance(chartData->totalMari, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZMARI) |
