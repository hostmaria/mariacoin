...
#include "qt/maria/forms/ui_addresseswidget.h"
#include "qt/maria/addressfilterproxymodel.h"
#include "qt/maria/addresslabelrow.h"
#include "qt/maria/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(MARIAGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto mariAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(mariAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(mariAdd);
        if (walletModel->updateAddressBookLabels(mariAdd, label.toUtf8().constData(),
