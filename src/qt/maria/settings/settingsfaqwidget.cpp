...
#include "config/maria-config.h"
#endif

#include "qt/maria/settings/settingsfaqwidget.h"
#include "qt/maria/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(MARIAGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableMARI,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableMARI,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableMARI,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("MARIA is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. MARIA incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("MARIA utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of MARIA to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableMARIContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received MARIA requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your MARIA wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableMARI->setText(unspendableMARIContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of MARIA with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("maria-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.MARIA.org'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::MARI)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in MARIA Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the MARIA network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableMARI, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableMARI);});
    ...
    if (parent)
        connect(parent, &MARIAGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
