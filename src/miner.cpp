...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("MARIAMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("MARIAMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MARIAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("maria-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MARIAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MARIAMiner exception");
    } catch (...) {
        LogPrintf("MARIAMiner exception");
    }

    LogPrintf("MARIAMiner exiting\n");
}
