MARIA Core version *v5.0.0* is now available from:  <https://github.com/Mariacoin/maria/releases>
...
MARIA Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, MARIA Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run MARIA v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving MARIs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://maria.org .
...
* Top bar showing shield MARI balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield MARI balance.
...
See the release notes from [v4.2.0](https://github.com/Mariacoin/MARIA/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed MARI supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in MARI
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in MARI
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zMARIsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield MARI)
  The `moneysupply` JSON return entry is the total MARI supply (the sum of all transparent and shield MARI)
...
| `getaccountaddress` | Returned the current MARIA address associated with an internal account | Account system removal |
| `setaccount` | Associate a MARIA address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given MARIA address | Account system removal |
| `getaddressesbyaccount` | Returned a list of MARIA addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a MARIA address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a MARIA address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zMARI balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zMARI mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zMARI mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zMARI amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zMARI to a MARI address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zMARI mints to a MARI address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zMARIs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzmariseed` | Set the wallet's deterministic zMARI seed to a specific value | Zerocoin deprecation |
| `getzmariseed` | Return the current deterministic zMARI seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zMARI seed | Zerocoin deprecation |
| `dzmaristate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzmari` | Do a search for deterministically generated zMARI that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_maria unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake mariad and unit tests: fixing missing MacOS framework. (furszy)
